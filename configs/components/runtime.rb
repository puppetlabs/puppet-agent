...
component "runtime" do |pkg, settings, platform|
  ...
  if platform.is_aix?
    ...
    pkg.install_file "/cygdrive/c/tools/mingw#{arch}/bin/libgcc_s_#{lib_type}-1.dll", settings[:bindir]
    pkg.install_file "/cygdrive/c/tools/mingw#{arch}/bin/libstdc++-6.dll", settings[:bindir]
    pkg.install_file "/cygdrive/c/tools/mingw#{arch}/bin/libwinpthread-1.dll", settings[:bindir]
    ...
    pkg.install_file "/cygdrive/c/tools/pl-build-tools/bin/zlib1.dll", settings[:bindir]
  else
