...
#  plat.service { :type => "sysv", :defaultdir => "/etc/sysconfig", :servicedir => "/etc/rc.d/init.d" }
  ...
  plat.provision_with "echo 'y' | yum install --nogpgcheck autoconf automake createrepo rsync gcc make rpmdevtools rpm-libs yum-utils rpm-sign rpm-build rsync; echo '[build-tools]\nname=build-tools\nbaseurl=http://enterprise.delivery.puppetlabs.net/build-tools/el/5/$basearch' > /etc/yum.repos.d/build-tools.repo"
